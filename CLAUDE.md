# CLAUDE.md — SNP Primer Windows App v13（ChatGPT 系列）

> 这份文件给未来的 Claude 会话用：你（Claude）会在 v13 目录里被启动协助迭代时，
> **先读这份文件**再动手。它只列那些**从代码里读不出来 / 容易踩坑 / 一旦忘记就出错**
> 的事。常规的目录布局、函数签名、依赖版本去 README.md 和代码里查。

> **v13 = v12 干净副本 + 多物种支持**（2026-05-21 同日）：用户问"如何让别的
> 物种的工作人员也能用？"v12 是小麦硬编码（`[1-7][ABD]` 正则 + getflanking
> ABD 过滤 + ploidy=3 缺省）。v13 抽出 `core/species.py` 集中物种配置，GUI 加
> Species 下拉（wheat 默认 + barley/rice/maize/sorghum/arabidopsis 五个预装），
> 选什么物种就驱动：
> - NCBI ENTREZ_QUERY taxid（`txid4565[ORGN]` / `txid4513[ORGN]` / ...）
> - `infer_chromosome` 正则（wheat `[1-7][ABD]` vs barley `[1-7]H` vs rice 纯
>   数字 1-12 vs ...）
> - `_extract_chrom_short` 截断长度（wheat/barley 2 字符 vs 其他完整 1-3 字符）
> - `getflanking.flanking` 的 ABD subgenome 过滤（wheat True，其它 False 跳过）
> - ploidy GUI 选项（wheat 1/2/3、其它只剩 1）
>
> **wheat 默认路径与 v12 字节等价**——`species_key="wheat"` 时所有改动都按
> pre-v13 行为兜底，Layer A/B 4/4 PASS 不能掉链。**`getflanking.py` 第一次
> 被改**（CLAUDE.md §2 "不要随便碰"清单里的文件），改动范围最小化：
> `_extract_chrom_short` 加 species 参数 + `flanking` 把 `if schrom[1] not in
> genomes` 条件化。详见 §6.22。
>
> v12 .git 历史不存在（v12 也是 cp-out + rm .git 拷的），v13 同样无嵌套 git。
> v12 在 priner_design 父目录里作为"小麦专用 + ENTREZ_QUERY"的冻结点保留。
> v11 早些时候已 push 到 GitHub `CrazyHsu/snp_primer_for_windows`，是远端基线。

> **v12 = v11 干净副本 + 在线 BLAST 小麦预过滤 + dropped hits 诊断日志**
> （2026-05-21）：用户报告选 `core_nt` 跑 NCBI 在线 BLAST 时
> `Dropped 210 hit(s) with no inferrable wheat chromosome` + `0 行 blast_out.txt`。
> 根因是 pipeline 硬编码小麦（`[1-7][ABD]` 正则）+ core_nt 大多数命中是非小麦
> 或小麦 cDNA/EST。v12 在 `run_ncbi_blast` 提交时默认带
> `ENTREZ_QUERY=txid4565[ORGN]`（NCBI Triticum aestivum taxid），让 NCBI 端就把
> 非小麦序列排除；`render_alignment_table_with_chrom_prefix` 日志改成
> total/kept/dropped + 前 5 条 sample dropped (subject_id, title)，方便用户
> 看清是 drop 合理还是正则漏匹配。**未动** `[1-7][ABD]` 正则、getflanking
> ABD 过滤、下游 getCAPS / getkasp3——小麦硬编码假设不变。详见 §6.21。
>
> v11 .git 历史**不继承**到 v12（拷贝时已删除嵌套 .git），v12 现在直接挂在
> 外层 `priner_design` 父 repo 里。v11 已在 GitHub
> `CrazyHsu/snp_primer_for_windows` 冻结，v12 作为实验分支验证 ENTREZ_QUERY
> 改动不破坏功能后再考虑是否合并回 v11 主线。
...
- **`core/getCAPS.py` / `getkasp3.py` / `getflanking.py` / `parse_polymarker_input.py`**：
  ...
  - **v13 例外**：`getflanking.py` 在 species 边界上做了最小化补丁（`_extract_chrom_short`
    + `flanking` 加 species 参数 + ABD 过滤条件化）。`species_key="wheat"`
    时仍走 pre-v13 路径字节等价，是用户选其它物种时才走新路径。如果未来还要
    再碰这个文件，请遵守同样的原则——wheat 默认不动、新功能用 `species` 参数
    带 default-None 兜底。详见 §6.22。
...
### 6.21 NCBI 在线 BLAST 自动加小麦物种过滤 + dropped hits 诊断日志（v12 主补丁）

**症状**（v11 上观察到）：用户在 GUI 选 `core_nt` 跑 NCBI 在线 BLAST，log 显示

```
Step 2: 在线 BLAST 模式=ncbi_online db=core_nt
Submitted NCBI BLAST RID=0WZX7ZK4014
Dropped 210 hit(s) with no inferrable wheat chromosome
Step 2 完成：写入 0 行 blast_out.txt
```

pipeline 卡死在 Step 3（temp_range.txt 是空的）。用户在 NCBI 状态 URL 上明明
看到有小麦命中条目。

**根因**：

1. **pipeline 硬编码小麦**——`online_blast.py:infer_chromosome` 用正则
   `[1-7][ABD]` 从 `subject_id + subject_title` 拼接里抓小麦 7×3 亚基因组
   （A/B/D）的染色体短码。这条假设跟整条下游链（getflanking ABD 过滤、
   getCAPS / getkasp3 的 ploidy=3 缺省）一起锁死，不能光改一处。
2. **core_nt 是 NCBI 核心核酸库**，返回 200+ hits 大多数是：
   - 非小麦物种（barley / rye / Aegilops / Brachypodium / 玉米 / 水稻 /
     拟南芥 / 病毒 / ...）
   - 小麦 cDNA / EST / 单基因克隆条目（subject_title 不带染色体名）
   - 偶尔的小麦基因组装但用非标准染色体记号（罕见）
3. **v11 的 dropped 日志只报数字**——用户没法判断 drop 的到底是非小麦还是
   小麦但正则漏匹配。

**修复**（v12，两改）：

(1) `run_ncbi_blast` 加默认 `entrez_query="txid4565[ORGN]"`：

```python
def run_ncbi_blast(..., entrez_query: str | None = "txid4565[ORGN]"):
    ...
    if entrez_query:
...
- `txid4565` 是 NCBI Triticum aestivum 的稳定 taxid（20+ 年没变过）。
- `[ORGN]` 是 Entrez organism 字段过滤符。
- `_http_post` 自动 urlencode `[` `]`，无需特殊处理。
- 这样 NCBI 端就只回小麦命中，client 端的 `[1-7][ABD]` 过滤变成"二道保险"，
  绝大多数 dropped 都不会出现。
- 跨物种切到大麦 / 水稻等需要同步改 (a) 这里的默认 (b) `infer_chromosome`
  正则 (c) getflanking ABD 过滤 (d) getCAPS / getkasp3 隐含的 ploidy 假设
  ——是大改不是小改。

(2) `render_alignment_table_with_chrom_prefix` 日志改造：

```python
def render_alignment_table_with_chrom_prefix(
    alignments, logger=None, *, sample_dropped_limit=5,
):
    ...
    kept = len(rewritten); dropped_samples = []
    if total:
        log_message(
            logger,
            f"BLAST hits: total={total}, kept (wheat chrom 1-7 A/B/D)={kept}, "
            f"dropped (no inferrable wheat chromosome)={dropped}",
        )
    if dropped_samples:
        log_message(logger, f"  Sample dropped hits (showing first {N}):")
        for sid, stitle in dropped_samples:
            log_message(logger, f"    - {sid}  |  {stitle}")
```

- 总数 / 保留 / drop 三元，比单 dropped 数字直观
- 前 5 条 sample dropped (subject_id, title) 让用户直接看到 drop 的具体是
  什么——如果都是非小麦或小麦无染色体的（如 cDNA），drop 合理；如果出现
  明显的小麦染色体记号（"Triticum aestivum chromosome 1A"）但仍被 drop，
  那就是正则漏匹配的真 bug
- subject_title 截断到 200 字符避免单条 hit 把 GUI Log tab 撑爆

**没动**：
- `[1-7][ABD]` 正则、`render_alignment_table_with_chrom_prefix` 的 chr 前缀
  改写逻辑、下游 getflanking / getCAPS / getkasp3 全部小麦硬编码
- `run_ebi_blast`——用户本轮只用 NCBI；EBI 的 organism 过滤参数语义不同
  （用 `taxids=4565` 而不是 `ENTREZ_QUERY`），等真用到 EBI online BLAST 撞
  类似问题再修
- GUI——不暴露 organism 输入框，跟 wheat-only 契约一致；用户若想自定义
  organism 可以在 `run_ncbi_blast` 调用时显式传 `entrez_query=None` 或
  其它 Entrez 表达式（需改 `core/pipeline.py` 透传，本轮没做）

**跨物种切换提醒**：`txid4565[ORGN]` 跟 `[1-7][ABD]` 正则是**一对**——一起改
才一致。单独改 ENTREZ_QUERY 让"NCBI 端只返回大麦"而正则仍按小麦匹配 → 全部
drop；单独改正则让"NCBI 仍返回小麦"而正则按大麦语义 → 也全部 drop。

排查钩子：
- log 里看到 "BLAST hits: total=N, kept=0, dropped=N" → 95% 是命中都没染色体
  名；看 sample dropped 行确认是非小麦还是小麦 cDNA。
- log 里看到 "Sample dropped hits" 里**有明显小麦染色体记号但仍被 drop** →
  真 bug，扩展 `infer_chromosome` 正则。用户能直接 paste sample 行给开发者。
- sample dropped 都不是小麦 → ENTREZ_QUERY 没生效，对 RID URL 里 NCBI 网页
  Search Summary 区显示的 Entrez Query 字段（如果是空的说明 submit_params 没
  注入；如果是 `txid4565[ORGN]` 但仍返非小麦说明 NCBI 端 Entrez index 没及时
  reindex——这种情况罕见，让用户 1 小时后重试或换 `nt_euk` 库）。
- 看到 "NCBI ENTREZ_QUERY filter: ..." log 行 → 确认 v12 的 entrez_query
  参数路径生效；如果没看到这行，要么是用了旧 `run_ncbi_blast` 二进制（venv
  里有旧 wheel），要么是 `entrez_query=None` 显式 override 了。

### 6.22 多物种支持（v13 主补丁）

**起因**：v12 在 NCBI 提交时默认带小麦 ENTREZ_QUERY，下游也假设 wheat 的
`[1-7][ABD]` 染色体记号 + ABD subgenome 过滤 + ploidy=3。用户问"如何让别的
物种（如水稻、玉米）的工作人员也能用？"v13 抽出物种配置 + GUI 加 Species
下拉，但保持 wheat 默认路径**字节等价**。

**架构**：所有物种硬编码集中在新文件 `core/species.py`：
...
class SpeciesConfig:
    ...
    display_name: str
    taxid: int
    entrez_query: str             # 喂给 run_ncbi_blast
    infer_chrom_patterns: tuple   # 喂给 render_alignment_table_with_chrom_prefix
    valid_chrom_codes: frozenset  # 校验用
    use_abd_filter: bool          # 喂给 getflanking.flanking
    ploidy_choices: tuple[int,...]  # 喂给 GUI Combobox
    default_ploidy: int

SPECIES_TABLE = {
    "wheat": ...,        # txid4565, [1-7][ABD], use_abd_filter=True,  ploidy 1/2/3
    "barley": ...,       # txid4513, [1-7]H,    use_abd_filter=False, ploidy 1
    "rice": ...,         # txid4530, 1-12,      use_abd_filter=False, ploidy 1
    "maize": ...,        # txid4577, 1-10,      use_abd_filter=False, ploidy 1
    "sorghum": ...,      # txid4558, 1-10,      use_abd_filter=False, ploidy 1
    "arabidopsis": ...,  # txid3702, 1-5,       use_abd_filter=False, ploidy 1
}
```

**6 处 wheat 硬编码每处怎么改的**（v12 → v13 对照）：

1. **`online_blast.py:infer_chromosome` 正则**：v12 是 wheat-only 函数；
   v13 deprecate 成 wheat-only alias，主调用走
   `core.species.infer_chromosome_for_species(text, species)`，正则从
   `species.infer_chrom_patterns` 取。
2. **`online_blast.py:_CHROM_PREFIXED_RE` 反解器**：v12 是
   `r"^chr([1-7][ABD])_(.+)$"`，只认小麦；v13 改成
   `r"^chr([A-Za-z0-9]{1,3})_(.+)$"`，跨物种工作。**不做** valid_chrom_codes
   硬卡——宽松反解让 pipeline 跑通比硬卡更友好。
3. **`getflanking.py:_extract_chrom_short`**：v12 截到 2 字符 `code[:2]`；
   v13 加 species 参数，`species is None or species.use_abd_filter` 时走
   pre-v13 路径（wheat byte-equiv），否则返回完整短码（rice "12"/"5"）。
4. **`getflanking.py:flanking:genomes="ABD"` 过滤**：v12 line 68-69 + line
   92 写死 ABD；v13 把 `genomes` 算法包到 `if species.use_abd_filter:` 分支，
   非小麦 `genomes=None` + 内层 `if genomes is not None: if schrom[1] not in
   genomes: continue` 跳过过滤。
5. **polymarker input 第二列染色体记号**：v12 用 "7A"（wheat）；v13 不改
   `parse_polymarker_input.py`，靠 `qchrom = qchrom[0:2]` 截前 2 字符的逻辑
   兼容 rice "10"→"10" / barley "5H"→"5H" / wheat "7A"→"7A"。**用户输入
   时染色体列要跟 species 实际命名一致**（GUI 不强制校验）。
6. **ploidy 缺省**：v12 GUI 写死 ["3", "2", "1"]；v13 `_on_species_changed`
   按 `species.ploidy_choices` 动态改 combo values + default。

**Wheat byte-equivalence 红线**：

任何 v13 代码路径在 `species_key="wheat"` 时必须与 v12 输出字节等价。验证
方法（WSL 端）：

```bash
cd chatgpt_snp_primer_app/snp_primer_windows_app_v13
PYTHONPATH=. python3 -c "
from pathlib import Path; from core import pipeline
fixt = Path('tests/fixtures').resolve()
pipeline.run(workdir='/tmp/v13_wheat_fixture',
    flanking_files=[str(p) for p in sorted((fixt/'flanking').glob('*.fa'))],
    alignment_files=[str(p) for p in sorted((fixt/'expected').glob('alignment_raw_*.fa'))],
    bin_dir='tests/.linux_bin', do_primer_blast=False,
    design_caps=True, design_kasp=True, max_tm=63, max_size=25, max_price=200,
    pick_anyway=0, ploidy=3, species_key='wheat')
"
python3 tests/compare_outputs.py --workdir /tmp/v13_wheat_fixture
# 必须 4/4 PASS
```

如果回归不通过 → 立刻删 v13 重来；v12 + v11 都在原位 + v11 已 push GitHub，
无数据丢失。

**非小麦 "best-effort" 边界**：

- 非小麦物种**不保证**逐字节匹配任何外部基准（没有 wheatomics 对照）。
- 只保证：(a) pipeline 跑完不崩 (b) Step 7 有输出文件 (c) chromosome 解析
  能命中至少一条 BLAST 命中。
- polymarker input 第二列必须用 species 实际命名（rice "10"、barley "5H"）。
- 用户的 `qchrom == schrom` 同染色体匹配规则（line 126 of getflanking.py）
  仍然生效，所以输入染色体写错就拿不到 snpinfo → temp_range.txt 空。

**切到第 7 个物种的步骤**：

1. 在 `core/species.py:SPECIES_TABLE` 加一条新 entry（taxid + 正则 +
   valid_codes + use_abd_filter + ploidy_choices）。
2. 不动 GUI 代码——`self.species_combo.values` 从 `list(SPECIES_TABLE.keys())`
   动态拿。
3. 跑非小麦 smoke 验证 BLAST 出至少 1 条 kept hit。
4. 如果是 polyploid 物种（如 cotton AD、棉花、草莓等）且要做 subgenome 区分，
   `use_abd_filter=True` 需要同步定义新的 subgenome 字母表（不能再用 "ABD"
   写死，要把 `flanking()` 里的 `"ABD"` 也参数化——这是更大的改动，不是单纯
   加表条目）。

**排查钩子**：

- 选了 species=X 但 log 里 NCBI ENTREZ_QUERY 还是 wheat → species 没透传
  到 `run_ncbi_blast`。检查 `core/pipeline.py:run()` 里
  `from core.species import get_species` + `entrez_query=species.entrez_query`
  这两行是不是被 import / 调用了。
- 非小麦 "BLAST hits: total=N, kept=0, dropped=N" → species 的
  `infer_chrom_patterns` 漏匹配，看 sample dropped 行的 subject_title 改正则。
- wheat 跑 Layer A 不再 4/4 PASS → 误改了 `getflanking.flanking` 的 wheat
  路径；用 `diff` v12 vs v13 的 `_extract_chrom_short` + `flanking` 函数对照，
  确认 wheat 行为完全 fall-through 到 pre-v13 代码块。
- `_on_species_changed` 没被调用（切 species 后 ploidy 没变）→ 检查
  `self.species_combo.bind("<<ComboboxSelected>>", ...)` 是不是真的 bind 上了。
- Reset Params 后 species 切回 wheat 但 ploidy 还是上次 diploid 的 "1" →
  检查 `reset_params` 里有没有调 `self._on_species_changed()`（v13 加了，
  v12 没这个调用）。

**EBI online BLAST 仍未做物种过滤**：用户当前场景是 NCBI；EBI 的 organism
参数语义不同（用 `taxids=4565`），等真用到再补。CLAUDE.md §6.21 也提过。
